...
embeddings_index = {}
with open('./pretrained_embeddings/glove.twitter.27B/glove.twitter.27B.200d.txt') as f:
    for line in f:
...
history = bilstm_cnn_words_model.fit(train_dataset_words, batch_size=BATCH_SIZE, epochs=EPOCHS, shuffle=True,
                              validation_data=valid_dataset_words, callbacks=[early_stop, reduce_lron_plateau,
                                                                   TensorBoard('./tensorboard/tb_logs_bilstm_cnn_glove_200')])
...
bilstm_cnn_words_dev_A_submissions.to_csv('./submissions/bilstm_cnn_glove_dev_A.csv', index=False)
...
bilstm_model = build_bilstm_model(lstm_units=64, dense_units=64)
bilstm_model.summary()

history = bilstm_model.fit(train_dataset_words, batch_size=BATCH_SIZE, epochs=EPOCHS, shuffle=True,
                              validation_data=valid_dataset_words, callbacks=[early_stop, reduce_lron_plateau,
                                                                   TensorBoard('./tensorboard/tb_logs_bilstm_glove_200')])

dev_test_preds = tf.cast(tf.greater(bilstm_model.predict(test_dataset), 0.5), tf.int64)

bilstm_words_dev_A_submissions = make_submissions(dev_test_preds, raw_test_data_taskA['rewire_id'])
bilstm_words_dev_A_submissions.to_csv('./submissions/bilstm_glove_dev_A.csv', index=False)
bilstm_words_dev_A_submissions.head()
...
def build_cnn_words_model(lstm_units=64, dense_units=256, dense_dropout=0.4):
  input = tf.keras.Input(shape=(1, ), dtype=tf.string, name='text') # input
  x = words_vectorizer_layer(input) # vectorize text 
  x = layers.Embedding(embedding_matrix.shape[0], WORDS_EMBEDDING_SIZE, weights=[embedding_matrix],
                            input_length=MAX_WORDS_SEQ_LENGTH,
...
cnn_words_model = build_cnn_words_model(lstm_units=128, dense_units=512)
cnn_words_model.summary()

history = cnn_words_model.fit(train_dataset_words, batch_size=BATCH_SIZE, epochs=EPOCHS, shuffle=True,
                              validation_data=valid_dataset_words, callbacks=[early_stop, reduce_lron_plateau,
                                                                   TensorBoard('./tensorboard/tb_logs_cnn_model_glove_200')])