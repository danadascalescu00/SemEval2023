# -*- coding: utf-8 -*-
"""GoogleNews300-Bi-LSTM-CNN_A.ipynb
...
history = bilstm_cnn_words_model.fit(train_dataset_words, batch_size=BATCH_SIZE, epochs=EPOCHS, shuffle=True,
                              validation_data=valid_dataset_words, callbacks=[early_stop, reduce_lron_plateau,
                                                                   TensorBoard('./tensorboard/tb_logs_bilstm_cnn_google_news_300')])
...
bilstm_cnn_words_dev_A_submissions.to_csv('./submissions/bilstm_cnn_google_news_dev_A.csv', index=False)
...
history = bilstm_model.fit(train_dataset_words, batch_size=BATCH_SIZE, epochs=EPOCHS, shuffle=True,
                              validation_data=valid_dataset_words, callbacks=[early_stop, reduce_lron_plateau,
                                                                   TensorBoard('./tensorboard/tb_logs_bilstm_google_news_300')])

dev_test_preds = tf.cast(tf.greater(bilstm_model.predict(test_dataset), 0.5), tf.int64)

bilstm_cnn_words_dev_A_submissions = make_submissions(dev_test_preds, raw_test_data_taskA['rewire_id'])
bilstm_cnn_words_dev_A_submissions.to_csv('./submissions/bilstm_google_news_dev_A.csv', index=False)
bilstm_cnn_words_dev_A_submissions.head()