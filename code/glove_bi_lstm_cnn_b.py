...
raw_train_data['label_category'].unique()

raw_train_data['label_category'].value_counts().plot(kind='bar')
...
history = bilstm_cnn_words_model.fit(train_dataset_words, batch_size=BATCH_SIZE, epochs=EPOCHS, shuffle=True,
                              validation_data=valid_dataset_words, callbacks=[early_stop, reduce_lron_plateau,
                                                                   TensorBoard('./tensorboard/tb_logs_bilstm_cnn_glove_200_B')])
...
bilstm_cnn_words_dev_B_submissions.to_csv('./submissions/bilstm_cnn_glove_dev_B.csv', index=False)
bilstm_cnn_words_dev_B_submissions.head()

"""# Glove-BiLSTM"""

def build_cnn_bilstm_model(lstm_units=64, dense_units=256, dense_dropout=0.4):
  input = tf.keras.Input(shape=(1, ), dtype=tf.string, name='text') # input
  x = words_vectorizer_layer(input) # vectorize text 
  x = layers.Embedding(embedding_matrix.shape[0], WORDS_EMBEDDING_SIZE, weights=[embedding_matrix],
                            input_length=MAX_WORDS_SEQ_LENGTH,
...
cnn_bilstm_model = build_cnn_bilstm_model(lstm_units=64, dense_units=64)
cnn_bilstm_model.summary()

history = cnn_bilstm_model.fit(train_dataset_words, batch_size=BATCH_SIZE, epochs=EPOCHS, shuffle=True,
                              validation_data=valid_dataset_words, callbacks=[early_stop, reduce_lron_plateau,
                                                                   TensorBoard('./tensorboard/tb_logs_bilstm_glove_200_B')])

dev_test_preds = cnn_bilstm_model.predict(test_dataset)

bilstm_words_dev_B_submissions = make_submissions(dev_test_preds, raw_test_data_taskA['rewire_id'])
bilstm_words_dev_B_submissions.to_csv('./submissions/bilstm_glove_dev_B.csv', index=False)
bilstm_words_dev_B_submissions.head()