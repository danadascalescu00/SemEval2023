...
clean_test_data = raw_test_data_taskA['text'].apply(clean_and_standardize_text)
...
chars_vocabulary = set()
for text in clean_train_data:
  chars_vocabulary.update(set([c for c in text]))
print(len(chars_vocabulary))
...
CHARS_MAX_FEATURES = len(chars_vocabulary)
CHARS_EMBEDDING_SIZE = 32
WORDS_EMBEDDING_SIZE = 32
BATCH_SIZE = 64
EPOCHS = 100

X_train, X_valid, y_train, y_valid = train_test_split(clean_train_data, raw_train_data['label_sexist'], test_size=0.25,
                                                      random_state=SEED, shuffle=True, stratify=raw_train_data['label_sexist'])

y_train = y_train.apply(lambda x: 1 if x == 'sexist' else 0).to_numpy().reshape(-1, 1)
y_valid = y_valid.apply(lambda x: 1 if x == 'sexist' else 0).to_numpy().reshape(-1, 1)
...
MAX_CHARS_SEQ_LENGTH = max(X_train.apply(lambda x: len(x))) + 1
...
def make_submissions(predictions, ids):
  df = pd.DataFrame(zip(ids, map(lambda x: 'sexist' if x else 'not sexist', predictions)), columns=['rewire_id', 'label_pred'])
  return df
...
def build_cnn_words_model(dense_units=256, dense_dropout=0.4):
  input = tf.keras.Input(shape=(1, ), dtype=tf.string, name='text') # tf.keras.Input(shape=(MAX_WORDS_SEQ_LENGTH, ), dtype="int64")
  x = words_vectorizer_layer(input) # vectorize text 
  x = layers.Embedding(WORDS_MAX_FEATURES + 1, WORDS_EMBEDDING_SIZE)(x) # get embeddings 
  ...
  conv_trigrams = layers.Conv1D(32, 3, padding='same', activation="relu", 
                                kernel_initializer='he_normal')(x)
  # # conv_tetragrams = layers.Conv1D(32, 4, padding='same', activation="relu", 
  # #                                 kernel_initializer='he_normal')(x)
  # # conv_pentagrams = layers.Conv1D(16, 5, padding='same', activation="relu", 
  # #                                 kernel_initializer='he_normal')(x)

  max_pool_bigrams = layers.MaxPool1D(pool_size=2)(conv_bigrams)
  max_pool_trigrams = layers.MaxPool1D(pool_size=2)(conv_trigrams)
  # # max_pool_tetragrams = layers.MaxPool1D(pool_size=2)(conv_tetragrams)
  # # max_pool_pentagrams = layers.MaxPool1D(pool_size=2)(conv_pentagrams)

  flatten_bigrams = layers.Flatten()(max_pool_bigrams)
  flatten_trigrams = layers.Flatten()(max_pool_trigrams)
  # # flatten_tetragrams = layers.Flatten()(max_pool_tetragrams)
  # # flatten_pentagrams = layers.Flatten()(max_pool_pentagrams)

  x = layers.concatenate([flatten_bigrams, flatten_trigrams]) #, flatten_tetragrams, flatten_pentagrams])
...
cnn_words_model = build_cnn_words_model(dense_units=512)
cnn_words_model.summary()

# keras.utils.plot_model(cnn_words_model, './cnn_words_model.png', show_shapes=True)

history = cnn_words_model.fit(train_dataset_words, batch_size=BATCH_SIZE, epochs=EPOCHS, shuffle=True,
                              validation_data=valid_dataset_words, callbacks=[early_stop, reduce_lron_plateau,
                                                                   TensorBoard('./tensorboard/tb_logs_cnn_words')])

dev_test_preds = tf.cast(tf.greater(cnn_words_model.predict(test_dataset), 0.5), tf.int64)

cnn_words_dev_A_submissions = make_submissions(dev_test_preds, raw_test_data_taskA['rewire_id'])
cnn_words_dev_A_submissions.to_csv('./submissions/cnn_words_dev_A.csv', index=False)
cnn_words_dev_A_submissions.head()

"""### CNN-chars"""

chars_vectorizer_layer = layers.TextVectorization(max_tokens=CHARS_MAX_FEATURES, split='character', output_sequence_length=MAX_CHARS_SEQ_LENGTH)
# Adapting the dataset
chars_vectorizer_layer.adapt(train_dataset.map(lambda x, y: x, num_parallel_calls=tf.data.AUTOTUNE).batch(BATCH_SIZE))

train_dataset_chars = train_dataset.batch(BATCH_SIZE)
valid_dataset_chars = valid_dataset.batch(BATCH_SIZE)
# test_ds = test_ds.cache().prefetch(buffer_size=AUTOTUNE)

def build_cnn_chars_model(dense_units=256, dense_dropout=0.4):
  input = tf.keras.Input(shape=(1, ), dtype=tf.string, name='text')
  x = chars_vectorizer_layer(input) # vectorize text 
  x = layers.Embedding(CHARS_MAX_FEATURES, CHARS_EMBEDDING_SIZE)(x) # get embeddings
  
  bigrams, trigrams, tetragrams, pentagrams = x, x, x, x
  for i in range(5):
    bigrams = layers.Conv1D(90, 2, padding='same', activation="relu", 
                                kernel_initializer='he_normal')(bigrams)
    bigrams = layers.MaxPool1D(pool_size=2)(bigrams)

    trigrams = layers.Conv1D(90, 3, padding='same', activation="relu", 
                                  kernel_initializer='he_normal')(trigrams)
    trigrams = layers.MaxPool1D(pool_size=2)(trigrams)

    tetragrams = layers.Conv1D(90, 4, padding='same', activation="relu", 
                                    kernel_initializer='he_normal')(tetragrams)
    tetragrams = layers.MaxPool1D(pool_size=2)(tetragrams)
    
    pentagrams = layers.Conv1D(90, 5, padding='same', activation="relu", 
                                    kernel_initializer='he_normal')(pentagrams)
    pentagrams = layers.MaxPool1D(pool_size=2)(pentagrams)


  flatten_bigrams = layers.Flatten()(bigrams)
  flatten_trigrams = layers.Flatten()(trigrams)
  flatten_tetragrams = layers.Flatten()(tetragrams)
  flatten_pentagrams = layers.Flatten()(pentagrams)
...
cnn_loop_model = build_cnn_chars_model(dense_units=512)
cnn_loop_model.summary()

history = cnn_loop_model.fit(train_dataset_chars, batch_size=BATCH_SIZE, epochs=EPOCHS, shuffle=True,
                              validation_data=valid_dataset_chars, callbacks=[early_stop, reduce_lron_plateau,
                                                                   TensorBoard('./tensorboard/tb_logs_cnn_chars')])

dev_test_preds = tf.cast(tf.greater(cnn_loop_model.predict(test_dataset), 0.5), tf.int64)

cnn_chars_dev_A_submissions = make_submissions(dev_test_preds, raw_test_data_taskA['rewire_id'])
cnn_chars_dev_A_submissions.to_csv('./submissions/cnn_chars_dev_A.csv', index=False)
cnn_chars_dev_A_submissions.head()
...
bilstm_cnn_words_model = build_bilstm_cnn_words_model(lstm_units=64, dense_units=512)
...
history = bilstm_cnn_words_model.fit(train_dataset_words, batch_size=BATCH_SIZE, epochs=EPOCHS, shuffle=True,
                              validation_data=valid_dataset_words, callbacks=[early_stop, reduce_lron_plateau,
                                                                   TensorBoard('./tensorboard/tb_logs_bilstm_cnn_words')])

dev_test_preds = tf.cast(tf.greater(bilstm_cnn_words_model.predict(test_dataset), 0.5), tf.int64)

bilstm_cnn_words_dev_A_submissions = make_submissions(dev_test_preds, raw_test_data_taskA['rewire_id'])
bilstm_cnn_words_dev_A_submissions.to_csv('./submissions/bilstm_cnn_words_dev_A.csv', index=False)
bilstm_cnn_words_dev_A_submissions.head()